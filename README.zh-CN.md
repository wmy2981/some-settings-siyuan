...
| `kernel-reconnect-button`   | 两端     | 与内核断开连接的面板上加一个「立即重连」按钮                                        |
| `kernel-auto-reconnect`     | 两端     | 断连时按设置的次数与间隔自己探测内核，探测到恢复就立刻重载前端（默认 2 次 / 500ms） |
| `mobile-slash-insert-panel` | 移动端   | 移动端输入 `/` 时用桌面端那套插入块面板，而不是键盘工具栏里的紧凑候选               |
...
* **总是显示标题块块标**：思源的块标不是由 CSS 控制显隐的，而是一个 protyle 一个
  `position: fixed` 的容器，由内核在鼠标悬浮 / 触摸时把当前块的块标渲染进去，
  离开时同时加 `fn__none` 并清空 `innerHTML`。纯 CSS 做不到常显，官方扩展点也没有；
  要真正常显只能由插件自己重绘一套块标并接管定位与交互。经确认本项跳过。
* **仅特定账号可用 DS 模型**、**DS 余额显示**、**简单的引用面包屑**：按要求暂不实现。
...
每个功能贡献一行小节标题（功能名与说明），随后是它自己的设置行，
行与行之间保留思源 `.b3-label` 自带的那条细分割线。**任何一层都没有嵌套分组** ——
...
文案占满一行、控件另起一行撑满宽度，同时收窄弹窗内边距，让内容用满整屏。
...
* **功能关着的时候不挂载**，因此它不能使用 `addTopBar` / `addDock` / `addTab` / `addCommand`
  这类必须在 onload 同步注册的 API。当前 24 个功能都没用到；将来要用的话，
